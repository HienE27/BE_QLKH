...
public class ReceiptOCRService {
     ...
     * Đồng thời trả về danh sách các phiếu tương tự để frontend có thể hiển thị gợi ý.
     */
    private void enrichWithVectorSearch(ReceiptOCRResponse response, List<Float> embedding, String receiptType) {
        try {
            ...
            // Danh sách dùng để trả về cho frontend
            List<ReceiptOCRResponse.SimilarReceipt> similarReceiptDtos = new ArrayList<>();

            // Lấy phiếu tương tự nhất (score thấp nhất = tương tự nhất với L2 distance)
            Map<String, Object> mostSimilar = similarReceipts.get(0);
            Double bestScore = (Double) mostSimilar.get("score");

            // Duyệt qua tất cả kết quả để:
            // - Gán danh sách phiếu tương tự vào response
            // - Sử dụng phiếu tốt nhất để tự động điền thông tin (nếu đủ tương tự)
            for (Map<String, Object> receiptResult : similarReceipts) {
                try {
                    Long id = receiptResult.get("id") instanceof Long ? (Long) receiptResult.get("id") : null;
                    Double score = (Double) receiptResult.get("score");
                    String metadataStr = (String) receiptResult.get("metadata");

                    String receiptTypeMeta = null;
                    String supplierName = null;
                    String customerName = null;
                    String receiptCode = null;
                    String receiptDate = null;
                    Double totalAmount = null;
                    String noteSummary = null;

                    if (metadataStr != null && !metadataStr.isEmpty()) {
                        JsonNode metadata = objectMapper.readTree(metadataStr);
                        if (metadata.has("receiptType")) {
                            receiptTypeMeta = metadata.get("receiptType").asText(null);
                        }
                        if (metadata.has("supplierName")) {
                            supplierName = metadata.get("supplierName").asText(null);
                        }
                        if (metadata.has("customerName")) {
                            customerName = metadata.get("customerName").asText(null);
                        }
                        if (metadata.has("receiptCode")) {
                            receiptCode = metadata.get("receiptCode").asText(null);
                        }
                        if (metadata.has("receiptDate")) {
                            receiptDate = metadata.get("receiptDate").asText(null);
                        }
                        if (metadata.has("totalAmount") && !metadata.get("totalAmount").isNull()) {
                            totalAmount = metadata.get("totalAmount").asDouble();
                        }
                        if (metadata.has("note")) {
                            String fullNote = metadata.get("note").asText("");
                            if (fullNote != null && !fullNote.isBlank()) {
                                noteSummary = fullNote.length() > 120 ? fullNote.substring(0, 117) + "..." : fullNote;
                            }
                        }

                        // Mapping sản phẩm: nếu có thông tin sản phẩm tương tự, có thể gợi ý productId
                        if (receiptResult == mostSimilar && metadata.has("products") && response.getProducts() != null) {
                    ...
                    ReceiptOCRResponse.SimilarReceipt dto = new ReceiptOCRResponse.SimilarReceipt(
                            id,
                            score,
                            receiptTypeMeta,
                            supplierName,
                            ...
                            totalAmount,
                            noteSummary);
                    similarReceiptDtos.add(dto);
                } catch (Exception e) {
                    log.warn("Failed to parse metadata from similar receipt", e);
                }
            }

            // Gán danh sách phiếu tương tự vào response (ngay cả khi score không đủ tốt để auto-fill)
            response.setSimilarReceipts(similarReceiptDtos);

            // Chỉ auto-fill khi phiếu tốt nhất đủ tương tự (score nhỏ hơn ngưỡng)
            if (bestScore != null && bestScore < 1.0) {
                log.info("Using best similar receipt to enrich response, score={}", bestScore);
                ...
                if (metadataStr != null && !metadataStr.isEmpty()) {
                    try {
                        ...
                        log.warn("Failed to apply best similar receipt metadata", e);
                    }
                }
            } else {
                log.info("Best similar receipt score too high ({}), skip auto-fill", bestScore);
            }
        } catch (Exception e) {
